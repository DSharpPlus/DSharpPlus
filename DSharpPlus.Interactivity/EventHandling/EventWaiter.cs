...
namespace DSharpPlus.Interactivity.EventHandling
{
    ...
    internal class EventWaiter<T> : IDisposable where T : AsyncEventArgs
    {
        ...
        public async Task<T> WaitForMatch(MatchRequest<T> request)
        {
            ...
            {
                this._client.Logger.LogError(InteractivityExtension.InteractivityEventId, ex, "An exception occurred while waiting for {0}", typeof(T).Name);
            }
            finally
            {
                request.Dispose();
        ...
        public async Task<ReadOnlyCollection<T>> CollectMatches(CollectRequest<T> request)
        {
            ...
            {
                this._client.Logger.LogError(InteractivityExtension.InteractivityEventId, ex, "An exception occurred while collecting from {0}", typeof(T).Name);
            }
            finally
            {
