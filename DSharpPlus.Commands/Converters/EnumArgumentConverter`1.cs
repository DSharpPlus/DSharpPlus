...
public class EnumConverter<T> : ISlashArgumentConverter<T>, ITextArgumentConverter<T>
    where T : struct, Enum
{
    ...
    public Task<Optional<T>> ConvertAsync(ConverterContext context)
    {
        ...
        // We need to ensure that the argument is the same type as it's enum base type.
        T value = (T)
            Convert.ChangeType(context.Argument, baseEnumType, CultureInfo.InvariantCulture);
        return Task.FromResult(
            Enum.IsDefined(value) ? Optional.FromValue(value) : Optional.FromNoValue<T>()
        );
    }
}
