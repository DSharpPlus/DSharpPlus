...
namespace DSharpPlus.Net.WebSocket
{
    ...
    public class WebSocketSharpClient : BaseWebSocketClient
    {
        ...
        {
            Socket = new wss.WebSocket(uri);

            Socket.OnOpen += (sender, e) => _connect.InvokeAsync().GetAwaiter().GetResult();

            Socket.OnClose += (sender, e) => _disconnect.InvokeAsync(new SocketCloseEventArgs(null) { CloseCode = e.Code, CloseMessage = e.Reason }).GetAwaiter().GetResult();
