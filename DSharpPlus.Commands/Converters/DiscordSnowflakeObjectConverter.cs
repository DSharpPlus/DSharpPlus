...
public partial class DiscordSnowflakeObjectConverter
    : ISlashArgumentConverter<SnowflakeObject>,
        ITextArgumentConverter<SnowflakeObject>
{
    ...
    public async Task<Optional<SnowflakeObject>> ConvertAsync(ConverterContext context)
    {
        ...
        if (
            await discordRoleSlashArgumentConverter.ConvertAsync(context)
                is Optional<DiscordRole> role
            && role.HasValue
        )
        {
        ...
        // Check if it's a member since it's more likely the command invoker wants to mention a member instead of a random person.
            ...
            await discordMemberSlashArgumentConverter.ConvertAsync(context)
                is Optional<DiscordMember> member
            && member.HasValue
        )
        {
        ...
        // Finally fallback to checking if it's a user.
            ...
            await discordUserSlashArgumentConverter.ConvertAsync(context)
                is Optional<DiscordUser> user
            && user.HasValue
        )
        {
