...
public partial class DiscordThreadChannelConverter
    : ISlashArgumentConverter<DiscordThreadChannel>,
        ITextArgumentConverter<DiscordThreadChannel>
{
    ...
    public Task<Optional<DiscordThreadChannel>> ConvertAsync(ConverterContext context)
    {
        ...
        if (!ulong.TryParse(value, CultureInfo.InvariantCulture, out channelId))
        {
            ...
            )
            {
                ...
                DiscordThreadChannel? namedChannel = context.Guild.Threads.Values.FirstOrDefault(
                    channel => channel.Name.Equals(value, StringComparison.Ordinal)
                );
                return Task.FromResult(
                    namedChannel is not null
                        ? Optional.FromValue(namedChannel)
                        : Optional.FromNoValue<DiscordThreadChannel>()
                );
            }
        }

        return
            context.Guild.Threads.TryGetValue(channelId, out DiscordThreadChannel? threadChannel)
            && threadChannel is not null
