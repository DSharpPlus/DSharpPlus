    ...
    >
{
    ...
    )
    {
        ...
        if (
            !TryFindCommand(
                ...
                out Command? command,
                out IReadOnlyList<DiscordInteractionDataOption>? options
            )
        )
        {
        ...
        {
            AutoCompleteContext? autoCompleteContext = await ParseAutoCompleteArgumentsAsync(
                converterContext
            );
            if (autoCompleteContext is not null)
            {
                foreach (Attribute attribute in autoCompleteContext.Parameter.Attributes)
                {
                    if (
                        attribute
                        is SlashAutoCompleteProviderAttribute autoCompleteProviderAttribute
                    )
                    {
                        await eventArgs.Interaction.CreateResponseAsync(
                            DiscordInteractionResponseType.AutoCompleteResult,
                            new DiscordInteractionResponseBuilder().AddAutoCompleteChoices(
                                await autoCompleteProviderAttribute.AutoCompleteAsync(
                                    autoCompleteContext
                                )
                            )
                        );
        ...
        IReadOnlyDictionary<CommandParameter, object?> parsedArguments = await ParseParametersAsync(
            converterContext
        );
        SlashCommandContext commandContext = CreateCommandContext(
            converterContext,
            parsedArguments
        );
            ...
            if (argument.Value is ArgumentFailedConversionResult argumentFailedConversionResult)
            {
                await this.extension.commandErrored.InvokeAsync(
                    ...
                    new CommandErroredEventArgs()
                    {
                        ...
                        CommandObject = null,
                        Exception = new ArgumentParseException(
                            argument.Key,
                ...
                await this.extension.commandErrored.InvokeAsync(
                    ...
                    new CommandErroredEventArgs()
                    {
                        ...
                        Exception = new ArgumentParseException(
                            argument.Key,
                            null,
                            "An earlier argument failed to parse, causing this argument to not be parsed."
                        ),
                    }
                );
    ...
    public bool TryFindCommand(
        ...
        [NotNullWhen(true)] out IReadOnlyList<DiscordInteractionDataOption>? options
    )
    {
        ...
        while (options.Any())
        {
            ...
            command = command.Subcommands.First(subcommandName =>
                this.Configuration.ParameterNamePolicy.GetCommandName(subcommandName) == option.Name
            );
    ...
    public async ValueTask ClearDiscordSlashCommandsAsync(bool clearGuildCommands = false)
    {
        ...
        await this.extension.Client.BulkOverwriteGlobalApplicationCommandsAsync(
    ...
    private async ValueTask<AutoCompleteContext?> ParseAutoCompleteArgumentsAsync(
        InteractionConverterContext converterContext
    )
    {
        if (this.extension is null)
        ...
        try
        {
            ...
            {
                DiscordInteractionDataOption? option = converterContext.Options.FirstOrDefault(
                    discordOption =>
                        discordOption.Name.Equals(
                            converterContext
                                .Extension.GetProcessor<SlashCommandProcessor>()
                                .Configuration.ParameterNamePolicy.GetParameterName(
                                    converterContext.Parameter,
                                    -1
                                ),
                ...
                IOptional optional = await this.ConverterDelegates[
                    IArgumentConverter.GetConverterFriendlyBaseType(converterContext.Parameter.Type)
                ](converterContext);
                parsedArguments.Add(
                    converterContext.Parameter,
                    optional.HasValue ? optional.RawValue : converterContext.Parameter.DefaultValue
                );
            }
            ...
            if (autoCompleteParameter is null || autoCompleteOption is null)
            {
                this.logger.LogWarning(
                    "Cannot find the auto complete parameter that the user is currently typing - this should be reported to library developers."
                );
                return null;
            }
        }
        catch (Exception error)
        {
            await this.extension.commandErrored.InvokeAsync(
                ...
                new CommandErroredEventArgs()
                {
                    ...
                    Exception = new ArgumentParseException(
                        converterContext.Parameter,
                        new() { Error = error, Value = converterContext.Argument?.RawValue }
                    ),
                    CommandObject = null,
                }
            );
        ...
        return new()
        {
            ...
            Extension =
                ...
                ?? throw new InvalidOperationException(
                    "SlashCommandProcessor has not been configured."
                ),
            Interaction = converterContext.Interaction,
            Options = converterContext.Options,
            ServiceScope = converterContext.ServiceScope,
