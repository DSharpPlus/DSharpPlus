---
uid: articles.basics.interactions.selects
title: Select Menus
---
...
Dropdowns are another [message component][0] added to messages, supported in all the places buttons are, however, dropdowns occupy an entire action row. Buttons cannot occupy the same row as a dropdown.
...
> [!NOTE]
> This article is written under the presumption that you are familiar with buttons. If you aren't, do familiarize yourself with the [documentation on buttons](buttons.md)
...
So lets go over these one by one, starting with the id. The id of a dropdown should of course be unique, just like buttons, and Discord will send this id back in the [interaction object][1].

Placeholder is also relatively relatively simple! It's hopefully self-explanatory, too. Placeholder text is the text the user will see when no options are selected. If you do not wish to have placeholder text, simply pass `null` as that parameter in the constructor for the dropdown. On string selects, placeholders only support plain text, and up to 100 characters.
...
Dropdown options are somewhat more involved than handling buttons, but they're still relatively simple. These consist of several parts:
...
Value is like the custom id of the dropdown for the most part it should be unique. This will be accessible on the @DSharpPlus.Entities.DiscordInteractionData.Values property the interaction, and will contain all the selected options.
...
// Make the dropdown
DiscordSelectComponent dropdown = new DiscordSelectComponent("dropdown", null, options, false, 1, 2);
```

Okay, so we have a dropdown...now what? Simply pass it to any builder that constructs a response, be it a @DSharpPlus.Entities.DiscordMessageBuilder, @DSharpPlus.Entities.DiscordInteractionResponseBuilder, or @DSharpPlus.Entities.DiscordWebhookBuilder.
...
## Selecting Discord API Entities

Select menus come in a number of other flavours: channel selects, user selects, role selects and 'mentionable' selects, which can be used to select either users or roles. In principle they function much the same as string selects, but their available values are determined by the API for you. You can specify default-selected values using the `AddDefault(s)` methods, and will receive the selected IDs back.

## Interactivity/Footnotes

"**Oh no, I'm getting 'This interaction failed' when selecting! What do I do?**"

Dropdowns are like buttons; when a user interacts with them, you need to respond to that interaction. `ComponentInteractionCreated` is fired from the client, just like buttons.

This applies to interactivity, too! Simply swap @DSharpPlus.Interactivity.Extensions.MessageExtensions.WaitForButtonAsync for @DSharpPlus.Interactivity.Extensions.MessageExtensions.WaitForSelectAsync, and pass a dropdown. How to go about component-based interactivity is described [in the buttons article][4].
