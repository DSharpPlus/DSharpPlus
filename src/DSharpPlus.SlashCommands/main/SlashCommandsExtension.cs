...
public sealed class SlashCommandsExtension : BaseExtension
{
    ...
    internal SlashCommandsExtension(SlashCommandsConfiguration configuration) => _configuration = configuration;
            ...
            foreach (Type type in types)
            {
                try
                {
                    ...
                    foreach (TypeInfo subclassinfo in classes)
                    {
                        ...
                        DiscordApplicationCommand payload = new DiscordApplicationCommand(groupAttribute.Name, groupAttribute.Description, defaultPermission: groupAttribute.DefaultPermission, allowDMUsage: allowDMs, defaultMemberPermissions: v2Permissions);
                    ...
                    void AddContextMenus(IEnumerable<MethodInfo> contextMethods)
                    {
                        foreach (MethodInfo contextMethod in contextMethods)
                        {
                            ...
                            DiscordApplicationCommand command = new DiscordApplicationCommand(contextAttribute.Name, null, type: contextAttribute.Type, defaultPermission: contextAttribute.DefaultPermission, allowDMUsage: allowDMUsage, defaultMemberPermissions: permissions);
    ...
    private ApplicationCommandOptionType GetParameterType(Type type)
    {
        ...
        return type == typeof(DiscordAttachment)
    ...
    private Task ContextMenuHandler(DiscordClient client, ContextMenuInteractionCreateEventArgs e)
    {
        _ = Task.Run(async () =>
        {
            ...
            try
            {
                ...
                //Gets the method for the command
                ContextMenuCommand? method = _contextMenuCommands.FirstOrDefault(x => x.CommandId == e.Interaction.Data.Id);
    ...
    internal async Task RunCommandAsync(BaseContext context, MethodInfo method, IEnumerable<object> args)
    {
        ...
        {
            await _slashInvoked.InvokeAsync(this, new SlashCommandInvokedEventArgs { Context = slashContext }, AsyncEventExceptionMode.ThrowAll);
        ...
        {
            await _contextMenuInvoked.InvokeAsync(this, new ContextMenuInvokedEventArgs() { Context = CMContext }, AsyncEventExceptionMode.ThrowAll);
    ...
    private async Task RunAutocomplete(DiscordInteraction interaction, ParameterInfo parameter, IEnumerable<DiscordInteractionDataOption> options, DiscordInteractionDataOption focusedOption)
    {
        ...
        try
        {
            ...
            object? providerInstance = Activator.CreateInstance(provider);
